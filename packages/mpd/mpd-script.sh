mpd_test=$(pgrep -f /data/data/com.webdefault.corks/files/usr/libexec/mpd)
CmdArgs=$(echo "$@")
#echo $mpd_test
mkdir -p ~/.mpd
...
if [ "x$CmdArgs" = x ] ; then
	if [ -z "$MULTIPLE_MPDS"  ] ; then
		...
		read answer
			...
			pgrep -f  /data/data/com.webdefault.corks/files/usr/libexec/mpd  | while read line; do kill  "$line"; done
			else 
			echo "doing nothing"
			exit
			fi
